...
function mostrarAlerta(texto, tipoDeAlerta) {
  ...
  document
    ...
    function registrarTarea() {
      const nombreDocente = document.querySelector(
        "input#alumno_first_name"
      ).value;
      const usuarioDocente = document.querySelector(
        "input#alumno_user_name"
      ).value;
      const passwordDocente = document.querySelector(
        "input#alumno_password"
      ).value;
      const nuevoDocente = new Docente(
        nombreAlumno,
        usuarioAlumno,
      ...
      const validadorPushNombre = validarNombre(nombreAlumno);
      const validadorPushUser = validarUsuario(usuarioAlumno, alumnoList);
      const validadorPushPass = validarPass(passwordAlumno);
