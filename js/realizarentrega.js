   ...
   // Si ya existe, el valor yaExisteEntrega se vuelve true
    for (let i = 0; i < dataUsuarioLogeado.entregas.length ;i++) { 
     if (dataUsuarioLogeado.entregas[i].idTarea === nuevaEntrega.idTarea) { 
       mostrarAlerta ("Error: no puede realizar más de una entrega para la misma tarea", "error")
    ...
    if (!yaExisteEntrega) { 
     ...
     getTarea(id_TareaActual, dataUsuarioLogeado.usuarioDocenteAsignado).entregas.push(
      nuevaEntrega
    )
